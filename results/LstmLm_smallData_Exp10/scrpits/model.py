...
class LstmLm(nn.Module):
    ...
    def generate_predictions(self, TEXT):
        ...
        data = torchtext.datasets.LanguageModelingDataset(
            ...
            text_field=TEXT)
        batch_number=1
        data_iter = torchtext.data.BPTTIterator(data, batch_number, 12, device=self.args.devid, train=False)
        ...
        with open(self.__class__.__name__ + ".preds.txt", "w") as f:
            ...
            for sentences in outputs:
                ...
                f.write( ''.join([x+' ' for x in input_words[0]]))
                f.write('\n \n ')
                
                f.write('expected output sentence : \n')
                f.write(''.join([x+' ' for x in output_words[0]]))
                f.write('\n \n ')
                
                f.write('sampled output sentence : \n')
                f.write(''.join([x+' ' for x in outputs[0]]))
                f.write('\n \n ')
